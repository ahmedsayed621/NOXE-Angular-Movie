import { Component, OnInit } from '@angular/core';

import {FormGroup , FormControl ,Validators} from '@angular/forms';

import {Router} from '@angular/router'

import {AuthService} from '../auth.service'

@Component({
  selector: 'app-register',
  templateUrl: './register.component.html',
  styleUrls: ['./register.component.scss']
})
export class RegisterComponent implements OnInit {

  error : string =""


  registerForm : FormGroup = new FormGroup( {

    first_name : new FormControl( null , [ Validators.required , Validators.minLength(2) , Validators.maxLength(10) ]  ),
    last_name : new FormControl( null , [ Validators.required , Validators.minLength(2) , Validators.maxLength(10) ]  ),
    email : new FormControl( null , [ Validators.required , Validators.email ]  ),
    password : new FormControl( null , [ Validators.required , Validators.pattern("^[a-zA-Z0-9]+$") ]  ),
    age : new FormControl( null , [ Validators.required , Validators.min(20) , Validators.max(60) ]  ),
   


  } )



  constructor(private _AuthService:AuthService , private _Router:Router) { }

  sendRegister(registerForm : FormGroup)
  {
    // console.log( registerForm.value   )// backend

    this._AuthService.registerToBack( registerForm.value ).subscribe((data)=>{

      if( data.message =="success" ) // register tmam
      {
           this._Router.navigate(['/login'])         // login
      }
      else
      {
        this.error = data.errors.email.message ;
      }


    } )
  }


  ngOnInit(): void {
  }

}
